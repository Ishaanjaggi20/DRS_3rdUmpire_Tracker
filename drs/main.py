...
def play(speed):
    ...
    if flag:
        canvas.create_text(136, 26, fill="red", font="Times 26 bold", text="Decision Pending")
    flag= not flag
...
# Controlling Buttons..
btn = tkinter.Button(window, text="<< Previous (fast)", width=40, command= partial(play, -15))
btn.pack()
...
btn = tkinter.Button(window, text="Next (fast) >>", width=40, command= partial(play, 15))
btn.pack()
